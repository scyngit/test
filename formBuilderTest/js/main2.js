  ...
  var options = {
    ...
    fieldRowColControl: function (fld) {
      ...
      function updateClassName() {
        ...
        const finalClass = ["form-control", rowClass, colVal]
          .filter(Boolean)
          .join(" ");
      ...
      const $labelRow = $rowGroup
        .closest(".input-wrap")
        .prev("label")
        .css({ "text-align": "left", "font-weight": "normal", width: "auto" });
      const $labelCol = $colGroup
        .closest(".input-wrap")
        .prev("label")
        .css({ "text-align": "left", "font-weight": "normal", width: "auto" });
      ...
      const $options = $(
        '<div class="form-group field-row-col-control"></div>'
      );
      const $label = $('<label class="false-label">横向排列</label>');
      const $optionswrap = $('<div class="sortable-options-wrap"></div>');
      const $ol = $('<ol class="sortable-options ui-sortable">').css({
        "border-radius": "4px",
      });
      const $li = $('<li class="ui-sortable-handle">').css({
        "border-radius": "4px",
        cursor: "default",
      });
      const $wrap = $('<div class="form-row-wrap">').css({
        display: "inline-block",
        "margin-left": "9px",
        "margin-right": "18px",
      });
      const $wrap1 = $('<div class="form-row-wrap">').css({
        display: "inline-block",
      });
    ...
    removeRowColClass: function (editPanel) {
      ...
      // 找到对应的 .prev-holder
      const prevHolder = document.querySelector(
        `.prev-holder[data-field-id="${fieldId}"]`
      );
